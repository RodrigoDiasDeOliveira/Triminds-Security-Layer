...
public class KafkaIdentityEventPublisher implements IdentityEventPublisherPort {
    ...
    public KafkaIdentityEventPublisher(KafkaTemplate<String, Object> kafka) { this.kafka = kafka; }

    private void publish(String type, Identity i) {
        Map<String, Object> payload = Map.of(
                "id", i.getId().toString(), "tenantId", i.getTenantId(),
                "username", i.getUsername(), "status", i.getStatus().name()
        );
