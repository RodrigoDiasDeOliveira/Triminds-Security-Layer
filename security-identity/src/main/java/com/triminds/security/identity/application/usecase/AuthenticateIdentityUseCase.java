...
public class AuthenticateIdentityUseCase {
    ...
    public AuthenticateIdentityUseCase(IdentityRepositoryPort identities, 
                                       CredentialRepositoryPort credentials,
                                       PasswordHasherPort hasher, 
                                       IdentityEventPublisherPort events) {
    ...
    public Identity execute(String tenantId, String username, String rawPassword) {
        ...
        if (i.getStatus() == IdentityStatus.LOCKED) 
            throw new DomainException("LOCKED", "Account locked");
        if (i.getStatus() == IdentityStatus.DISABLED) 
            throw new DomainException("DISABLED", "Account disabled");
